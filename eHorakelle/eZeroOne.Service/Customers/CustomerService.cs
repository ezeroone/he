...
namespace eZeroOne.Service.Customers
{
    public class CustomerService : ICustomerService
    {
        ...
        {
            var cust = _repository.Single<Client>(t => t.UserId == client.ClientId);
            if (cust == null)
            {
        ...
        {
            var cust = _repository.Single<Client>(t => t.ClientId == client.ClientId);
            if (cust != null)
            {
                cust.Discount = client.Discount;
                _unitOfWork.Commit();
                return true;
            }
            return false;
        ...
        {
            return (from e in _repository.All<Client>()
                    select e).AsEnumerable();
        }

        public Client GetClient(int id)
        {
            return _repository.Single<Client>(t => t.ClientId == id);
        }

        public Client GetClientUserId(int id)
        {
            return _repository.Single<Client>(t => t.UserId == id);
        }
        public Visitor GetVisitorUserId(int id)
        {
            return _repository.Single<Visitor>(t => t.UserId == id);
        }
    }
}
